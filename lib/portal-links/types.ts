...
import { comparePriority } from "@/lib/ordering";
...
// Las primitivas de orden viven en `lib/ordering.ts`: las comparte con la
// selección del cliente, que resuelve exactamente el mismo problema.
export { orderByRating, reorderIds } from "@/lib/ordering";

/** Orden de trabajo de la lista de enlaces. */
export function compareByPriority(
  a: { position: number | null; created_at: string },
  b: { position: number | null; created_at: string },
): number {
  return comparePriority(a, b, (x) => x.created_at);
}
