...
var require_helpers = __commonJS({
  "node_modules/@supabase/auth-js/dist/main/lib/helpers.js"(exports) {
    ...
    function retryable(fn, isRetryable) {
      const promise = new Promise((accept, reject2) => {
        ;
        (async () => {
  ...
  "node_modules/@supabase/auth-js/dist/main/lib/locks.js"(exports) {
    ...
    async function processLock(name, acquireTimeout, fn) {
      ...
      const currentOperation = (async () => {
        let timeoutId = null;
        try {
          const timeoutPromise = acquireTimeout >= 0 ? new Promise((_, reject2) => {
            timeoutId = setTimeout(() => {
              ...
              reject2(new ProcessLockAcquireTimeoutError(`Acquiring process lock with name "${name}" timed out`));
            }, acquireTimeout);
          }) : null;
...
var handleError = async (error, reject2, options, namespace) => {
  if (error !== null && typeof error === "object" && "json" in error && typeof error.json === "function") {
    ...
    responseError.json().then((err) => {
      ...
      reject2(new StorageApiError(_getErrorMessage(err), status, statusCode, namespace));
    }).catch(() => {
      const statusCode = status + "";
      reject2(new StorageApiError(responseError.statusText || `HTTP ${status} error`, status, statusCode, namespace));
    });
  } else reject2(new StorageUnknownError(_getErrorMessage(error), error, namespace));
};
...
async function _handleRequest(fetcher, method, url, options, parameters, body, namespace) {
  return new Promise((resolve, reject2) => {
    fetcher(url, _getRequestParams(method, options, parameters, body)).then((result) => {
      ...
      return result.json();
    }).then((data) => resolve(data)).catch((error) => handleError(error, reject2, options, namespace));
  });
}
...
// node_modules/@supabase/supabase-js/dist/index.mjs
var import_auth_js = __toESM(require_main2(), 1);
...
// lib/services/story/structure.ts
var NON_ENTITY = /* @__PURE__ */ new Set([
  "la",
  "el",
  "los",
  "las",
...
function unsupportedEntities(copy, supportTexts) {
...
var ENGINE_MODEL_MIN_DATE = "2026-08-20T23:00:00Z";
...
var { data: versions } = await db.from("property_story_versions").select("id, property_id, created_at, model, provider").eq("status", "generated").gte("created_at", ENGINE_MODEL_MIN_DATE).order("created_at", { ascending: true });
console.log(`[publish] ${versions?.length ?? 0} versiones generadas con engine v4`);
var rejects = {};
var rejectExamples = {};
var published = [];
function reject(gate, ref) {
...
for (const v of versions ?? []) {
  ...
  if (prop.archived_at || prop.status === "archived") {
    reject("propiedad no disponible", ref);
    continue;
  }
  if (blocks.some((b) => b.status === "conflict")) {
    reject("1\xB7bloque en conflicto", ref);
    continue;
  }
  const chapters = blocks.map((b) => b.chapter);
  if (new Set(chapters).size !== chapters.length) {
    reject("2\xB7cap\xEDtulo duplicado", ref);
    continue;
  }
  const allClaimIds = blocks.flatMap((b) => b.claim_ids ?? []);
  if (new Set(allClaimIds).size !== allClaimIds.length) {
    reject("3\xB7claim reutilizado", ref);
    continue;
  }
  if (blocks.some((b) => copyWordCount(b.copy) > 70)) {
    reject("4\xB7bloque >70 palabras", ref);
    continue;
  }
  ...
  if (tooShort.length > 0) {
    reject("5\xB7bloque demasiado corto", ref);
    continue;
  }
  ...
  for (const b of blocks) {
    ...
    if (unsupportedEntities(b.copy, support).length > 0) {
      entityFail = true;
      break;
    }
  }
  if (entityFail) {
    reject("6\xB7entidad sin respaldo / bloque sin evidencia", ref);
    continue;
  }
  ...
  if (floor === 0 && /planta baja[^.]*\b(trastero|garaje|gimnasio|almacen|zonas? comunes)\b/i.test(prop.description ?? "")) {
    reject("7\xB7planta inferida de zona secundaria", ref);
    continue;
  }
  ...
  if (photoFail) {
    reject("8\xB7foto incoherente con cap\xEDtulo", ref);
    continue;
  }
  ...
  if (barrioBlock) {
    ...
    if (hood) {
      ...
      if (!zoneOk) {
        reject("9\xB7barrio incoherente con zone/subzone", ref);
        continue;
      }
    }
  }
  if (blocks.some((b) => !b.copy || !b.copy.trim())) {
    reject("15\xB7heading vac\xEDo", ref);
    continue;
  }
  ...
  if (blocks.some((b) => BOILER.test(b.copy))) {
    reject("16\xB7boilerplate de agencia en el copy", ref);
    continue;
  }
  const narrative = blocks.filter((b) => ["living", "kitchen", "private", "outdoor", "finishes", "building"].includes(b.chapter));
  if (narrative.length < 3) {
    reject("10\xB7menos de 3 cap\xEDtulos narrativos", ref);
    continue;
  }
  if ((photos?.length ?? 0) < 8) {
    reject("cobertura fotogr\xE1fica baja (<8)", ref);
    continue;
  }
  if (!DRY_RUN) {
    ...
    await db.from("property_story_versions").update({
      ...
      notes: "Rollout controlado 50 \xB7 quality gate autom\xE1tico (10 criterios) superado."
    }).eq("id", v.id);
  }
  published.push({ ref, slug: prop.slug, zone: prop.subzone || prop.zone, blocks: blocks.length, photos: photos?.length ?? 0 });
  console.log(`\u2713 ${published.length}/${TARGET} ${ref} \xB7 ${prop.subzone || prop.zone} \xB7 ${blocks.length} bloques \xB7 ${photos?.length ?? 0} fotos \xB7 ${prop.slug}`);
}
console.log(`
...
for (const [gate, n] of Object.entries(rejects).sort((a, b) => b[1] - a[1])) {
  console.log(`   ${n.toString().padStart(4)} \xB7 ${gate} (ej. ${rejectExamples[gate]})`);
}
