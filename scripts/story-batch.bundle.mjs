...
function validateClaims(claims, facts) {
  ...
  for (const claim of claims) {
    ...
    const sqmOf = (t) => t.match(/(\d{2,4})\s*m²|(\d{2,4})\s*m2\b|(\d{2,4})\s*metros/i);
    ...
    if (!claim.conflict) {
      ...
      const dupSqm = !!sqmFact && !!facts.squareMeters && Math.abs(Number(sqmFact[1] ?? sqmFact[2] ?? sqmFact[3]) - facts.squareMeters) / facts.squareMeters <= 0.05;
...
async function generateStoryForProperty(propertyId) {
  const db2 = createAdminClient();
  const { data: row, error: error2 } = await db2.from("properties").select("id, description, bedrooms, bathrooms, square_meters, zone, subzone, features, features_manual, title").eq("id", propertyId).maybeSingle();
  if (error2 || !row) return { ok: false, error: "Propiedad no encontrada" };
...
// scripts/run-story-batch.mts
var LIMIT = Number(process.argv[2] ?? 0) || null;
var CONCURRENCY = 2;
var PAUSE_MS = 400;
var MAX_CONSECUTIVE_FAILURES = 15;
var db = createAdminClient();
var { data: rows, error } = await db.from("properties").select("id, bc_reference, slug, published_web, description").is("archived_at", null).neq("status", "archived").order("published_web", { ascending: false }).order("last_synced_at", { ascending: false, nullsFirst: false });
if (error) {
  console.error("FATAL: no se pudo listar el inventario:", error.message);
  process.exit(1);
}
var words = (t) => ((t ?? "").match(/\S+/g) ?? []).length;
var queue = (rows ?? []).filter((r) => words(r.description) >= 25);
var targets = LIMIT ? queue.slice(0, LIMIT) : queue;
console.log(`[batch] ${(/* @__PURE__ */ new Date()).toISOString()} \xB7 ${targets.length} propiedades en cola (de ${rows?.length ?? 0} activas)`);
var done = 0;
var storiesNew = 0;
var storiesReused = 0;
var storiesFailed = 0;
...
async function processOne(row, idx) {
  ...
  try {
    ...
    if (gen.ok) {
      ...
      console.log(
        `${tag} \xB7 fotos +${cls.classified}/${cls.skipped}skip/${cls.failed}err \xB7 story ${gen.reused ? "cacheada" : `NUEVA (${gen.blocks} bloques, ${gen.conflicts} conflictos)`}`
      );
    } else {
      consecutiveFailures++;
      storiesFailed++;
      console.log(`${tag} \xB7 fotos +${cls.classified} \xB7 story ERROR: ${gen.error}`);
    }
  } catch (err) {
    consecutiveFailures++;
    storiesFailed++;
    console.log(`${tag} \xB7 EXCEPCI\xD3N: ${err instanceof Error ? err.message : String(err)}`);
  }
  done++;
  if (done % 25 === 0) {
    console.log(
      `[progreso] ${done}/${targets.length} \xB7 nuevas:${storiesNew} cacheadas:${storiesReused} fallos:${storiesFailed} \xB7 fotos clasificadas:${photosClassified} \xB7 conflictos:${conflictsTotal}`
    );
  }
  if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
    console.error(`[batch] ABORTADO: ${MAX_CONSECUTIVE_FAILURES} fallos consecutivos (\xBFIA ca\xEDda?). Relanzar cuando se resuelva \u2014 es reanudable.`);
    process.exit(2);
  }
}
var cursor = 0;
...
console.log(
  `[batch] FIN ${(/* @__PURE__ */ new Date()).toISOString()} \xB7 ${done} procesadas \xB7 stories nuevas:${storiesNew} cacheadas:${storiesReused} fallos:${storiesFailed} \xB7 fotos clasificadas:${photosClassified} \xB7 conflictos detectados:${conflictsTotal} \xB7 NADA publicado (todo en 'generated')`
);
