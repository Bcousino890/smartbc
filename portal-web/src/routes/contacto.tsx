...
export const Route = createFileRoute("/contacto")({
  head: () => ({
    meta: [
      { title: "Contacto — Benjamín Cousiño Propiedades · Madrid & Santiago" },
      { name: "description", content: "Hablemos. Un asesor de nuestra oficina en Madrid o Santiago le presentará las mejores oportunidades en menos de 24 horas." },
      { property: "og:title", content: "Contacto — Benjamín Cousiño Propiedades" },
      { property: "og:description", content: "Personal shopper inmobiliario · España y Chile." },
    ],
  }),
  component: Contacto,
});
        ...
        <div className="bg-card border border-border p-10 lg:p-14">
          {sent ? (
            <div className="text-center py-16">
              ...
              <p className="mt-4 text-muted-foreground">Nos pondremos en contacto en menos de 24 horas con información personalizada.</p>
            </div>
          ) : (
            <>
              ...
              <form className="mt-10 space-y-6" onSubmit={(e) => { e.preventDefault(); setSent(true); }}>
                <div className="grid sm:grid-cols-2 gap-6">
                  <F label="Nombre completo *"><input required className="w-full border-b border-border bg-transparent py-3 focus:border-gold outline-none" /></F>
                  <F label="Email *"><input required type="email" className="w-full border-b border-border bg-transparent py-3 focus:border-gold outline-none" /></F>
                  <F label="Teléfono"><input className="w-full border-b border-border bg-transparent py-3 focus:border-gold outline-none" /></F>
                  <F label="País de interés">
                    <select className="w-full border-b border-border bg-transparent py-3 focus:border-gold outline-none">
                ...
                <F label="Mensaje *"><textarea required rows={5} className="w-full border-b border-border bg-transparent py-3 focus:border-gold outline-none resize-none" /></F>
        ...
        <aside className="space-y-8">
          ...
          {[
            ...
            <div key={o.city} className="border-l-2 border-gold pl-5">
              ...
              <p className="text-sm text-muted-foreground mt-2">{o.addr}</p>
              <a href={`tel:${o.phone.replace(/\s/g, "")}`} className="block mt-3 text-navy hover:text-gold font-medium">{o.phone}</a>
              <p className="mt-1 text-[11px] tracking-[0.2em] uppercase text-muted-foreground">{o.hours}</p>
            </div>
          ))}
          ...
          <div className="border border-gold/40 p-6 bg-cream-deep mt-12">
            ...
            <a href="/off-market" className="mt-4 inline-flex items-center gap-2 text-[11px] tracking-[0.24em] uppercase text-gold hover:text-navy">
