      ...
      <section className="relative min-h-[70vh] flex items-center overflow-hidden">
        ...
        <div className="container-luxe relative z-10 py-32 text-cream">
          ...
          <p className="mt-8 max-w-2xl text-cream/80 leading-relaxed">
            Residencias, villas y pisos en Madrid, Santiago y otras ubicaciones exclusivas que no encontrará en ningún portal.
          </p>
        </div>
      </section>
      ...
      <section className="container-luxe py-24 grid lg:grid-cols-2 gap-16 items-start">
        <div>
          ...
          <p className="mt-8 text-navy/80 leading-relaxed">
            Muchos propietarios de inmuebles premium prefieren vender o alquilar con absoluta discreción. Tras más de 7 años en el sector, nuestra oficina ha construido una red que da acceso directo a este mercado invisible.
          </p>
          <div className="mt-12 grid grid-cols-2 gap-px bg-stone-200">
            <div className="bg-cream py-10 px-6">
              <p className="font-display text-6xl text-gold">1.800+</p>
              <p className="mt-3 text-[11px] tracking-[0.24em] uppercase text-gray-400">Propiedades activas</p>
            </div>
            <div className="bg-cream py-10 px-6">
              <p className="font-display text-6xl text-gold">60%</p>
              <p className="mt-3 text-[11px] tracking-[0.24em] uppercase text-gray-400">Operaciones off market</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-cream-deep p-10 lg:p-14 border-t-2 border-gold">
          ...
          <p className="mt-3 text-sm text-gray-500">Complete su perfil y un asesor le presentará las oportunidades disponibles.</p>

          {sent ? (
            <div className="mt-8 border border-gold p-8 text-center">
              <p className="font-display text-2xl text-navy">Solicitud recibida</p>
              <p className="mt-3 text-sm text-gray-500">Un asesor de nuestra oficina le contactará en menos de 24 horas.</p>
            </div>
          ) : (
            <form className="mt-8 space-y-5" onSubmit={(e) => { e.preventDefault(); setSent(true); }}>
              <Field label="Nombre *"><input required className="w-full bg-cream border border-stone-200 px-4 py-3 focus:border-gold outline-none" /></Field>
              <Field label="Email *"><input required type="email" className="w-full bg-cream border border-stone-200 px-4 py-3 focus:border-gold outline-none" /></Field>
              <Field label="Teléfono"><input className="w-full bg-cream border border-stone-200 px-4 py-3 focus:border-gold outline-none" /></Field>
              <Field label="Operación">
                <select className="w-full bg-cream border border-stone-200 px-4 py-3 focus:border-gold outline-none">
              ...
              <Field label="Descripción de su búsqueda"><textarea rows={4} className="w-full bg-cream border border-stone-200 px-4 py-3 focus:border-gold outline-none resize-none" /></Field>
